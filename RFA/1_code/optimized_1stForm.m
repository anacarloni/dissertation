...
% Primeira Forma dos Polinômios Interpoladores
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
form = '1st';
...
de = 1e-5;
dx = 1e-5;
options = optimset('MaxIter',itermax,'TolFun',de,'TolX',dx,'MaxFunEvals',...
    funmax,'PlotFcns',@optimplotfval);
warning('off','all') % já sabemos que a matriz pode ser próxima de singular

% Otimização dos Polos e Coeficientes Lineares (simplex Nelder-Mead)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fun = @(x) fiterror_1stForm(x,H,M,k,kk);
[Bf,fminopt] = fminsearch(fun,Bini,options);
[~,Fc,kFc,C] = fiterror_1stForm(Bf,H,M,k,kk);
disp('Polos otimizados:');
disp(Bf');

% Visualização da Função Objetivo
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% figures_objFun
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
addpath('figures');
dir = [fileparts(pwd),filesep,'3_output/optimized_1stForm/' num2str(Np) '_poles\'];
figures_tf

% Comparação dos Resultados - Plano Complexo
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
kflutter = 0.1826;
id = find(k>=kflutter,1);
switch caso
    case {'04'}
        aux=20;
    case {'05'}
...
filename = [fileparts(pwd),filesep,...
    sprintf('2_pipeline/optimized_1stForm/%s_poles',num2str(Np))];
