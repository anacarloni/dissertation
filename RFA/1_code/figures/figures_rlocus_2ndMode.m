% Plotanto os Local das Raízes do Segundo Modo
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
f2 = figure;
...
xlim([-0.4 .1]);
ylim([5.25 5.4]);
xlabel('\sigma_{2}/\omega_{r}')
ylabel('\omega_{2}/\omega_{r}')
grid on
hold on
...
cc = jet(length(U));
plot(sigma_ref(:,4),wn_ref(:,4),'^','MarkerSize',13,'MarkerFaceColor', ...
    cc(20,:),'MarkerEdgeColor','k','HandleVisibility','off');
plot(sigma_ref(:,5),wn_ref(:,5),'^','MarkerSize',13,'MarkerFaceColor', ...
    cc(50,:),'MarkerEdgeColor','k','HandleVisibility','off');
plot(sigma_ref(:,6),wn_ref(:,6),'^','MarkerSize',13,'MarkerFaceColor', ...
    cc(80,:),'MarkerEdgeColor','k','HandleVisibility','off');

legend_entries{(length(U)/aux+1)} = ' ';
cont=1;
i_U = 1;
...
figname = sprintf('Case%s_rLocus_2ndMode',caso);
figures_save